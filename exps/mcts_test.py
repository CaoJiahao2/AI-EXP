...
def visualize_grid(grid, path=None, search_tree=None, start=(0,0), goal=(0,0), filename="grid_path.jpg", obstacle_density=0.2):
    ...
    ax.legend()
    plt.gca().invert_yaxis()  # 将y轴反转，使左下角为原点
    plt.title(f"Grid Path Planning (Size: {rows}x{cols}, Obstacle Density: {obstacle_density})")
...
def run_experiment(rows=10, cols=10, obstacle_density=0.2, iterations=10000, save_visualization=True):
    """
    运行一次MCTS路径规划实验
    :param rows: 网格行数
    :param cols: 网格列数
...
def main():
    ...
    for exp in experiments:
        print("\n" + "="*50)
        print(f"运行实验: {exp}")
        run_experiment(rows=exp['rows'], cols=exp['cols'], obstacle_density=exp['obstacle_density'], iterations=500000)
